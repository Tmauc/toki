...
class _UsageStatsPageState extends State<UsageStatsPage> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.black,
        title: Text(context.l10n.usageStatistics),
        centerTitle: true,
        elevation: 0,
      ...
      body: Consumer<UsageProvider>(
        builder: (context, provider, child) {
          ...
          return TabBarView(
            controller: _tabController,
            children: [
              _buildUsageListView(provider.todayUsage, provider.todayHistory, 'today', _screenshotKeys[0], provider),
              _buildUsageListView(
                  provider.monthlyUsage, provider.monthlyHistory, 'monthly', _screenshotKeys[1], provider),
              _buildUsageListView(provider.yearlyUsage, provider.yearlyHistory, 'yearly', _screenshotKeys[2], provider),
              _buildUsageListView(
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Text(context.l10n.noDataYet, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
          ...
          child: ListView(
            ...
            children: [
              ...
              _buildUsageCard(
                ...
                value: '${numberFormatter.format(stats.wordsTranscribed)} ${context.l10n.understandingWords}',
  ...
  Widget _buildChart(List<UsageHistoryPoint> history, String period) {
    ...
    switch (period) {
      case 'today':
        ...
        processedHistory = List.generate(24, (hour) {
          if (hourlyMap.containsKey(hour)) return hourlyMap[hour]!;
      ...
      case 'monthly':
        ...
        processedHistory = List.generate(daysInMonth, (i) {
          final day = i + 1;
          if (dailyMap.containsKey(day)) return dailyMap[day]!;
      ...
      case 'yearly':
        ...
        processedHistory = List.generate(12, (i) {
          final month = i + 1;
          if (monthlyMap.containsKey(month)) return monthlyMap[month]!;
      ...
      case 'all_time':
        ...
        processedHistory = years.map((year) {
          if (yearlyMap.containsKey(year)) return yearlyMap[year]!;
    ...
    for (var point in processedHistory) {
      ...
      if (_isMetricVisible[1] && point.wordsTranscribed.toDouble() > maxY) maxY = point.wordsTranscribed.toDouble();
      if (_isMetricVisible[2] && point.insightsGained.toDouble() > maxY) maxY = point.insightsGained.toDouble();
      if (_isMetricVisible[3] && point.memoriesCreated.toDouble() > maxY) maxY = point.memoriesCreated.toDouble();
    }
    maxY = maxY * 1.2;
    if (maxY == 0) maxY = 1;
          ...
          getTooltipItems: (List<LineBarSpot> touchedBarSpots) {
            return touchedBarSpots
                .map((barSpot) {
                  ...
                  final originalIndex = metricColors.indexOf(barSpot.bar.color!);
                  ...
                  return LineTooltipItem(
                    ...
                    children: [
                      TextSpan(
                        text: NumberFormat.compact(locale: 'en_US').format(barSpot.y),
    ...
    return Wrap(
      ...
      children: List.generate(legendItems.length, (index) {
        return _buildLegendItem(
          ...
          () => setState(() => _isMetricVisible[index] = !_isMetricVisible[index]),
        );
      }),
    );
  }
